...
static int fusb302_convert_bc_lvl(int port, int bc_lvl)
{
...
static int fusb302_measure_cc_pin_source(int port, int cc_measure)
{
	int switches0_reg = 0;
	int reg = 0;
	int cc_lvl = 0;
...
static void fusb302_detect_cc_pin_source_manual(int port, int *cc1_lvl, int *cc2_lvl)
{
	int cc1_measure = TCPC_REG_SWITCHES0_MEAS_CC1;
	int cc2_measure = TCPC_REG_SWITCHES0_MEAS_CC2;
//  APP_DEBUG("C%d SRC vconn_enabled %d cc_polarity %d", port, state[port].vconn_enabled, state[port].cc_polarity);

	if (state[port].vconn_enabled) {
		/* If VCONN enabled, measure cc_pin that matches polarity */
		if (state[port].cc_polarity)
			*cc2_lvl = fusb302_measure_cc_pin_source(port, cc2_measure);
		else
			*cc1_lvl = fusb302_measure_cc_pin_source(port, cc1_measure);
	} else {
...
static void fusb302_detect_cc_pin_sink(int port, int *cc1, int *cc2)
{
	int reg;
	int orig_meas_cc1;
	int orig_meas_cc2;
	int bc_lvl_cc1;
	int bc_lvl_cc2;
//	APP_DEBUG("C%d SINK vconn_enabled %d cc_polarity %d", port, state[port].vconn_enabled, state[port].cc_polarity);
	/*
	 * Measure CC1 first.
	 */
	tcpc_read(port, TCPC_REG_SWITCHES0, &reg);
...
//	platform_usleep(250);
	...
	*cc1 = fusb302_convert_bc_lvl(port, bc_lvl_cc1);
	*cc2 = fusb302_convert_bc_lvl(port, bc_lvl_cc2);
...
static int fusb302_tcpm_send_message(int port, uint16_t header, const uint32_t *data,
				 uint8_t *buf, int buf_pos)
{
  ...
  if (port == 2 || port == 0) {
//    APP_DEBUG("C%d buf_pos = %d", port, buf_pos);
...
{
//  APP_DEBUG("Port %d pulling_up %d", port, state[port].pulling_up);
	if (state[port].pulling_up) {
		/* Source mode? */
		fusb302_detect_cc_pin_source_manual(port, cc1, cc2);
	} else {
		/* Sink mode? */
		fusb302_detect_cc_pin_sink(port, cc1, cc2);
	}
...
static int fusb302_tcpm_set_cc(int port, int pull)
{
  ...
  APP_DEBUG("C%d vconn %d cc_polarity %d pulling_up %d pull %d", port, state[port].vconn_enabled, state[port].cc_polarity, state[port].pulling_up, pull);
//  APP_DEBUG("C%d cc_polarity %d", port, state[port].cc_polarity);
//  APP_DEBUG("C%d pulling_up %d pull %d", port, state[port].pulling_up, pull);
//  APP_DEBUG("C%d rx_enable %d", port, state[port].rx_enable);
	switch (pull) {
	case TYPEC_CC_RP:
      /* turn off toggle */
...
//  APP_DEBUG("C%d vconn %d", port, state[port].vconn_enabled);
//  APP_DEBUG("C%d cc_polarity %d polarity %d snk_sel_pin %d", port, state[port].cc_polarity, polarity, usb_pd_snk_sel_pin);
//  APP_DEBUG("C%d pulling_up %d", port, state[port].pulling_up);
//  APP_DEBUG("C%d rx_enable %d", port, state[port].rx_enable);
...
//        digitalWrite(usb_pd_snk_sel_pin, LOW);
        gpio_set_level((gpio_num_t)usb_pd_snk_sel_pin, 0);
      }
      break;
    case 1:
      if (polarity == 0) {  // unflipped
...
//        digitalWrite(usb_pd_src1_sel_pin, LOW);
        gpio_set_level((gpio_num_t)usb_pd_src1_sel_pin, 0);
      }
      break;
  }
#else
...
//        digitalWrite(usb_pd_src2_sel_pin, LOW);
        gpio_set_level((gpio_num_t)usb_pd_src2_sel_pin, 0);
      }
      break;
    case 3:
      if (polarity == 0) {
...
//        digitalWrite(usb_pd_src3_sel_pin, LOW);
        gpio_set_level((gpio_num_t)usb_pd_src3_sel_pin, 0);
      }
      break;
  }
#endif
...
static int fusb302_tcpm_set_vconn(int port, int enable)
{
  ...
  APP_DEBUG("C%d vconn %d enable %d", port, state[port].vconn_enabled, enable);
//  APP_DEBUG("C%d cc_polarity %d", port, state[port].cc_polarity);
//  APP_DEBUG("C%d pulling_up %d", port, state[port].pulling_up);
//  APP_DEBUG("C%d rx_enable %d", port, state[port].rx_enable);
...
{
	int reg;
//  APP_DEBUG("C%d power_role %d data_role %d", port, power_role, data_role);
...
//  APP_DEBUG("C%d cc_polarity %d", port, state[port].cc_polarity);
//  APP_DEBUG("C%d pulling_up %d", port, state[port].pulling_up);
//  APP_DEBUG("C%d rx_enable %d enable %d cc_polarity %d vconn_enabled %d", port, state[port].rx_enable, enable, state[port].cc_polarity, state[port].vconn_enabled);
...
static int fusb302_tcpm_get_message(int port, uint32_t *payload, int *head)
{
  ...
  {
//    APP_DEBUG("Port %d fifo is empty", port);
		return EC_ERROR_UNKNOWN;
  }
...
//    rv |= tcpc_xfer(port, 0, 0, buf, 3, I2C_XFER_START);
    ...
    if (port == 2 || port == 0) {
//      APP_DEBUG("C%d Buf[0] = 0x%02X", port, buf[0]);
//      APP_DEBUG("C%d Buf[1] = 0x%02X", port, buf[1]);
//      APP_DEBUG("C%d Buf[2] = 0x%02X", port, buf[2]);
    }
    ...
    if (port == 2 || port == 0) {
//      APP_DEBUG("C%d rv %d len from head = %d", port, rv, len);
    }
    ...
    if (port == 2 || port == 0) {
//      APP_DEBUG("C%d rv %d head TYPE %d CNT %d Buf = ", port, rv, PD_HEADER_TYPE(*head), PD_HEADER_CNT(*head));
...
//      printf("\r\n");
	...
	if (!rv) {
		...
		if (PACKET_IS_GOOD_CRC(*head)) {
			rv = EC_ERROR_UNKNOWN;
//            APP_DEBUG("C%d get GoodCRC package", port);
		} else {
...
//	    delayMicroseconds(1200);   // mike 20211118
        delayMicroseconds(600);  // mike 20220425 debug
...
static int fusb302_tcpm_get_vbus_level(int port)
{
	...
	/* Read status register */
	tcpc_read(port, TCPC_REG_STATUS0, &reg);
  APP_DEBUG("C%d TCPC_REG_STATUS0 %d ", port, reg);
...
//      APP_DEBUG("C%d interrupt %d interrupta %d interruptb %d", port, interrupt, interrupta, interruptb);
    }
  } else {
    if (interrupt > 0 || interrupta > 0 || interruptb > 0) {
//      APP_DEBUG("C%d interrupt %d interrupta %d interruptb %d", port, interrupt, interrupta, interruptb);
...
//      APP_DEBUG("C%d interrupt TCPC_REG_INTERRUPT_COLLISION", port);
    ...
    if (interrupta & TCPC_REG_INTERRUPTA_TX_SUCCESS) {
      //task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_RX, 0);
//      APP_DEBUG("C%d interrupt TCPC_TX_COMPLETE_SUCCESS", port);
      pd_transmit_complete(port, TCPC_TX_COMPLETE_SUCCESS);
    }

    if (interrupta & TCPC_REG_INTERRUPTA_RETRYFAIL) {
      /* all retries have failed to get a GoodCRC */
//      APP_DEBUG("C%d interrupt TCPC_REG_INTERRUPTA_TX_SUCCESS", port);
      pd_transmit_complete(port, TCPC_TX_COMPLETE_FAILED);
    }
...
//      APP_DEBUG("C%d interrupt TCPC_REG_INTERRUPTA_HARDSENT", port);
...
//        APP_DEBUG("C%d interrupt TCPC_REG_INTERRUPTA_HARDRESET", port);
        fusb302_pd_reset(port);
...
//      APP_DEBUG("C%d interrupt TCPC_REG_INTERRUPTB_GCRCSENT", port);
      if (state[port].rx_enable) {
        //task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_RX, 0);
      } else {
