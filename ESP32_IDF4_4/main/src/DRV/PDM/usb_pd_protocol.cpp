...
#define CPRINTF(format, args...) APP_DEBUG(format, ## args)
#define CPRINTS(format, args...) APP_DEBUG(format, ## args)
...
static uint8_t pd_try_src_enable = 0;
#endif
...
static int send_control(int port, int type)
{
	...
	if (debug_level >= 2)
		CPRINTF("C%d CTRL[%d]>%d", port, type, bit_len);
...
static int send_source_cap(int port)
{
	...
	if (debug_level >= 2)
		CPRINTF("C%d srcCAP>%d", port, bit_len);
...
static int send_battery_cap(int port, uint32_t *payload)
{
	...
	if (debug_level >= 2)
		CPRINTF("C%d batCap>%d", port, bit_len);
	return bit_len;
}
...
static int send_battery_status(int port,  uint32_t *payload)
{
	...
	if (debug_level >= 2)
		CPRINTF("C%d batStat>%d", port, bit_len);
...
static void send_sink_cap(int port)
{
	...
	if (debug_level >= 2)
		CPRINTF("C%d snkCAP>%d", port, bit_len);
}
...
static int send_request(int port, uint32_t rdo)
{
	...
	if (debug_level >= 2)
		CPRINTF("C%d REQ>%d", port, bit_len);
...
static int send_bist_cmd(int port)
{
	...
	CPRINTF("C%d BIST>%d", port, bit_len);
...
//        }
	...
	case PD_DATA_VENDOR_DEF:
		...
		break;
	default:
		CPRINTF("C%d Unhandled data message type %d", port, type);
	}
}
...
#endif
		CPRINTF("C%d Unhandled ctrl message type %d", port, type);
	}
}
		 ...
		 int count)
{
	if (count > VDO_MAX_SIZE - 1) {
		CPRINTF("C%d VDM over max size", port);
		return;
	}
...
// DECLARE_HOOK(HOOK_BATTERY_SOC_CHANGE, pd_update_try_source, HOOK_PRIO_DEFAULT);  // mike disabled
#endif
...
{
  CPRINTS("TCPC C%d init Begin..", port);
  ...
  int res = 0;
	...
	CPRINTS("TCPC C%d init res = %d %s", port, res, res ? "failed" : "ready");
...
//  CPRINTS("TCPC C%d VID:0x%x PID:0x%x DID:0x%x FWV:0x%lx", port, info->vendor_id, info->product_id, info->device_id, info->fw_version_number);
	}
#endif
  ...
  CPRINTS("TCPC C%d power_role %d", port, pd[port].power_role);
...
void pd_run_state_machine(int port, int reset)
{
  ...
  if (evt != 0) {
    CPRINTF("C%d event = %02X", port, evt);
  }
...
//    hard_reset_sent[port] = 0;
    CPRINTS("TCPC C%d reset!", port);
    if (tcpm_init(port) != EC_SUCCESS)
      CPRINTS("TCPC C%d init failed", port);
#ifdef CONFIG_USB_PD_DUAL_ROLE_AUTO_TOGGLE
  }
...
//      CPRINTF("C%d flags is no PD_FLAGS_PING_ENABLED", port);
	...
	case PD_STATE_SUSPENDED: {
		...
		int rstatus;
#endif
		CPRINTS("TCPC C%d suspended!", port);
		...
		if (rstatus != 0 && rstatus != EC_ERROR_UNIMPLEMENTED)
			CPRINTS("TCPC C%d release failed!", port);
#endif
		/* Wait for resume */
		// getting rid of task stuff
...
//        CPRINTF("C%d pd cc_state = %d", port, pd[port].cc_state);
		tcpm_get_cc(port, &cc1, &cc2);
...
//        CPRINTF("C%d cc1 = %d cc2 = %d new cc = %d", port, cc1, cc2, new_cc_state);
...
//        CPRINTF("C[%d].polarity = %d", port, pd[port].polarity);
...
#if defined(CONFIG_CHARGE_MANAGER)
		typec_curr = get_typec_current_limit(pd[port].polarity, cc1, cc2);
		//typec_set_input_current_limit(port, typec_curr, TYPE_C_VOLTAGE);
#endif
		/* If PD comm is enabled, enable TCPC RX */
		if (pd_comm_is_enabled(port))
			tcpm_set_rx_enable(port, 1);
		...
		/* DFP is attached */
		if (new_cc_state == PD_CC_DFP_ATTACHED || new_cc_state == PD_CC_DEBUG_ACC) {
			pd[port].flags |= PD_FLAGS_CHECK_PR_ROLE | PD_FLAGS_CHECK_DR_ROLE | PD_FLAGS_CHECK_IDENTITY;
			if (new_cc_state == PD_CC_DEBUG_ACC)
				pd[port].flags |= PD_FLAGS_TS_DTS_PARTNER;
...
//    CPRINTF("C%d cc1 = %d cc2 = %d new typec_curr = %d", port, cc1, cc2, typec_curr);
	...
	case PD_STATE_DRP_AUTO_TOGGLE:
	{
		...
		if (next_state != PD_STATE_DRP_AUTO_TOGGLE) {
			...
			CPRINTS("TCPC C%d Exit Low Power Mode", port);
#endif
		}
		...
		if (next_state == PD_STATE_SNK_DISCONNECTED) {
			...
			timeout = -1;
#ifdef CONFIG_USB_PD_TCPC_LOW_POWER
			CPRINTS("TCPC C%d Low Power Mode", port);
#endif
		}
...
void pd_set_suspend(int port, int enable)
{
	...
	if (enable) {
		...
		if (!tries)
			CPRINTS("TCPC C%d set_suspend failed!", port);
	} else {
		if (pd[port].task_state != PD_STATE_SUSPENDED)
			CPRINTS("TCPC C%d suspend disable request "
