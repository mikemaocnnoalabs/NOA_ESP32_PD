...
int pd_board_check_request(uint32_t, int)
	__attribute__((weak, alias("stub_pd_board_check_request"))); */  // mike disabled
...
//      CPRINTF("pdo[%d]=%d %d mv: %d ma", i, pd_src_pdo[i], ((pd_src_pdo[i] >> 10) & 0x3ff) * 50, (pd_src_pdo[i] & 0x3ff) * 10);
    ...
    if (bpower_save == 0) {
	  pd_source_cap_current_index = pd_find_pdo_index(port, PD_MAX_VOLTAGE_MV, &pdo);  // Ask max voltage in default
    } else {
      pd_source_cap_current_index = pd_find_pdo_index(port, PD_MIN_VOLTAGE_MV, &pdo);  // Ask min voltage before powersave
    }
	pd_extract_pdo_power(pdo, &ma, &mv);
    CPRINTF("C[%d] cap current index %d mv: %d ma: %d", port, pd_source_cap_current_index, mv, ma);
