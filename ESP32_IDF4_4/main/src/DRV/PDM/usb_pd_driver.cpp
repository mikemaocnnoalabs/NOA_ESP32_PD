...
uint32_t pd_src_pdo[PDO_MAX_OBJECTS] = {
  ...
  PDO_FIXED(20000, 3250, PDO_FIXED_FLAGS),
};
...
void pd_power_supply_reset(int port)
{
 ...
 if (port == 2) {
  CPRINTF("Reset C%d Vbus Power", port);
  }
...
{
  CPRINTF("Off C%d Vbus Power", port);
...
#else
  if (port == 2) {
    CPRINTF("Enable C%d Vbus Power", port);
  }
  if(port < 1 || port > 3) {  // support 1 - 3 port only
    return EC_ERROR_UNKNOWN;
  }
...
//  CPRINTF("Transition %d Port voltage", port);
  ...
  CPRINTF("C%d Current requested index: %d %d mV %d mA", port, idx, pdo_mv, pdo_ma);
