...
//  DBGLOG(Info, "index: %d totle: %d", j, number_of_array_elements);
  if (j >= number_of_array_elements) {
//    DBGLOG(Info, "number %d index: %d totle: %d", number, j, number_of_array_elements);
    databuf = NULL;
  }
  // unlock
  xSemaphoreGive(parameter_system_mutex);
...
//      DBGLOG(Info, "para[%d] number is %ld str %s", j, number, str);
      switch (number) {
        ...
        case 52:
        case 56:
          DBGLOG(Info, "para_meter %d can't be set", number);
          ret = 0x02;
          break;
        default:
          break;
...
//        DBGLOG(Info, "PARA %d=%s", NOA_PDT[j].num, str);
        memory_release(str);
        break;
      }
      DBGLOG(Info, "para_meter %ld is ready", number);
      memory_release(str);
    }
    if (j >= number_of_array_elements) {
      DBGLOG(Error, "para_meter %ld is wrong", number);
      ret = 0x03;
    }
  }
...
void NOA_Parameter_Update(void) { // Update para with detault value for OTA
  ...
  DBGLOG(Info, "Checking NOA_Parameter_Update %d!", number_of_array_elements);
  for (j = 0; j < number_of_array_elements; j++) {
  ...
  if (ret_update == 1) {
    DBGLOG(Info, "NOA_Parameter_Update for version update!");
  }
}

int NOA_SysPara_init(void) {
  DBGLOG(Info, "NOA_SysPara_init");
  Para_Init_flag = 0;
  // Create para mutex
...
void NOA_Parametr_Default(void) {   // Reset NOA_PDT table from Default_Para table
  DBGLOG(Debug, "NOA_Parametr_Default");
