...
//      DBGLOG(Info, "Para[%d] Response:%s", nTemp_i, NOAServer.arg((nTemp_i)).c_str());
      if (nFile_Size == 0 && nTemp_i == 1) {
        nFile_Size = NOA_PUB_Swap_charNum((char *)NOAServer.arg((nTemp_i)).c_str());
//        DBGLOG(Info, "nFile_Size %d", nFile_Size);
      }
    }
  } else {
    NOAServer.send(200);
...
void handleFileupload() {  // callback
  if (bUpdate == false) {
    DBGLOG(Error, "No in Upgrade Status!");
    return;
  }
...
//    DBGLOG(Info, "Begin upload file:%s %d %d %d", upload.filename.c_str(), upload.currentSize, upload.totalSize, nFile_Counts);
    if (nFile_Size == 0) {
      ...
      DBGLOG(Info, "FreeSketchSpace size %d", maxSketchSpace);
      if (!Update.begin(maxSketchSpace, U_FLASH)) {//start with max available size
        DBGLOG(Error, "Update begin error");
...
//    DBGLOG(Info, "uploaded %d %d %d bytes of file size ", upload.currentSize, upload.totalSize, nFile_Counts);
    ...
    DBGLOG(Info, "Write %d %d %d bytes to OTA partition",upload.currentSize, upload.totalSize, nFile_Counts);
...
//  WiFi.disconnect();
  ...
  DBGLOG(Info, "Scan Done");
  if (n == 0) {
    DBGLOG(Info, "No networks found");
  } else {
    DBGLOG(Info, "%d WIFI(2.4G)Networks Found", n);
    for (int i = 0; i < n; ++i) {
