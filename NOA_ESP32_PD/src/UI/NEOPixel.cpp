...
void RGB_Light_Test_Task_Loop( void * pvParameters ){
  DBGLOG(Info, "RGB_Light_Test_Task_Loop running on core %d", xPortGetCoreID());
  NEO_Pixel_Clear();
...
void RGB_Light_Task_Loop( void * pvParameters ){
  DBGLOG(Info, "RGB_Light_Task_Loop running on core %d", xPortGetCoreID());
...
//    msg = 0;
    ...
    if (xStatus == pdPASS) {
//      DBGLOG(Info, "RGB_Light_Task_Loop get a message %d QueueSpaces %d", msg.message, uxQueueSpacesAvailable(NOA_RGB_TASK));
    }
    switch(msg.message) {
      case RGB_MSG_READY:
        break;
...
// Create the task without using any dynamic memory allocation.
  ...
  if (RGB_Light_Test_Task == NULL || &xTaskBuffer_RGB_Light_Test == NULL) {
    DBGLOG(Error, "Create RGB_Light_Test_Task fail");
  }
...
//  NOA_RGB_TASKQUEUE = xQueueCreate(SIZE_OF_TASK_QUEUE, sizeof(uint32_t));
  ...
  if (NOA_RGB_TASKQUEUE == NULL) {
    DBGLOG(Error, "Create NOA_RGB_TASKQUEUE fail");
  }
  ...
  if (RGB_Light_Task == NULL || &xTaskBuffer_RGB_Light == NULL) {
    DBGLOG(Error, "Create RGB_Light_Task fail");
  }
}
